...
describe("Todo Application", function () {
  ...
  test("Creates a todo", async () => {
    ...
    const parsarfind = JSON.parse(response.text);
    expect(parsarfind.id).toBeDefined();
  });
  ...
  test("Marks a todo as complete", async () => {
    ...
    const parsarfind = JSON.parse(response.text);
    const todoVK = parsarfind.id;

    expect(parsarfind.completed).toBe(false);
  ...
  test("Fetches all todos", async () => {
    ...
    expect(parsarfind.length).toBe(4);
    expect(parsarfind[3]["title"]).toBe("Buy ps3");
  });
  ...
  test("Deletes a todo", async () => {
    ...
    const deleteTodoResponses = await agent.delete(`/todos/${todoVK}`).send();
